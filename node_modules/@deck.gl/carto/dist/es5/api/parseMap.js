    ...
    layers: extractTextLayers(layers.reverse()).map(function (_ref) {
      ...
      try {
        ...
        return new Layer(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread(_objectSpread({
          id: id,
          data: data
        }, defaultProps), !config.textLabel && createInteractionProps(interactionConfig)), styleProps), createChannelProps(visualChannels, type, config, data)), createParametersProp(layerBlending, styleProps.parameters || {})), createLoadOptions(token)));
      } catch (e) {
  ...
  if (type === 'point') {
    ...
    if (altitude) {
      result.dataTransform = function (data) {
        data.features.forEach(function (_ref3) {
